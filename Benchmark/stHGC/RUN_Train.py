"""
stHGC — Self-supervised Triplet Hypergraph Convolutional Network for Spatial Transcriptomics
...
from stHGC.Train import stHGC_train


def mclust_R(adata, n_clusters, use_rep='stHGC', key_added='mclust', random_seed=0):
...
def main():
    ...
    adata = stHGC_train(adata, n_epochs=500, device=device, key_added='stHGC', random_seed=2020)
    adata = mclust_R(adata, n_clusters, use_rep='stHGC')
    ...
    print(f"stHGC {sample}: ARI = {ARI:.4f}")
