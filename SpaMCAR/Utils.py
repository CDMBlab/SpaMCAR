...
np.random.seed(3407)
random.seed(3407)
sc.settings.seed = 3407
def res_search(adata, target_k=7, res_start=0.1, res_step=0.1, res_epochs=10, config=None):
    """
    ...
    res = res_start
    sc.tl.leiden(adata, resolution=res, random_state=config.get('seed', 3407))
    ...
    while old_k != target_k:
        ...
        sc.tl.leiden(adata, resolution=res + res_step * old_sign, random_state=config.get('seed', 3407))
...
def refine_label(adata, radius=50, key='cluster', threshold=0.3):
    ...
    for i in range(n_cell):
        neighbors = np.argsort(distance[i])[1:radius + 1]
        neighbor_labels = old_type[neighbors]
        unique, counts = np.unique(neighbor_labels, return_counts=True)
        
        max_count = np.max(counts)
        if max_count / len(neighbors) >= threshold:  # confidence threshold met, update label
            new_type.append(unique[np.argmax(counts)])
